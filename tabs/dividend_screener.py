...
def process_dividend_history(dividend_data, daily_price_data, overview_data):
    ...
    quarterly_dividends = (
        df.groupby(["year", "quarter", "year_quarter"])["dividend"].sum().reset_index()
    )
    quarterly_dividends["date"] = pd.to_datetime(
        quarterly_dividends["year"].astype(str)
        + "-"
        + (quarterly_dividends["quarter"] * 3).astype(str)
        + "-01"
    )
    ...
    if current_price > 0:
        df["estimated_yield"] = (
            df["dividend"] * 4 / current_price
        ) * 100  # Rough quarterly estimate
        annual_dividends["estimated_yield"] = (
            annual_dividends["dividend"] / current_price
        ) * 100
    else:
...
def calculate_financial_metrics(
    overview_data, income_data, balance_data, cash_flow_data
):
    ...
    try:
        ...
        metrics["shares_outstanding"] = safe_float(
            overview_data.get("SharesOutstanding", 0)
        )
        ...
        metrics["dividend_yield"] = (
            safe_float(overview_data.get("DividendYield", 0)) * 100
        )
        metrics["dividend_per_share"] = safe_float(
            overview_data.get("DividendPerShare", 0)
        )
        ...
        metrics["price_to_sales"] = safe_float(
            overview_data.get("PriceToSalesRatioTTM", 0)
        )
        ...
        metrics["profit_margin"] = (
            safe_float(overview_data.get("ProfitMargin", 0)) * 100
        )
        metrics["operating_margin"] = (
            safe_float(overview_data.get("OperatingMarginTTM", 0)) * 100
        )
        ...
        metrics["revenue_growth_yoy"] = (
            safe_float(overview_data.get("QuarterlyRevenueGrowthYOY", 0)) * 100
        )
        metrics["earnings_growth_yoy"] = (
            safe_float(overview_data.get("QuarterlyEarningsGrowthYOY", 0)) * 100
        )
        ...
        metrics["debt_to_equity"] = safe_float(
            overview_data.get("DebtToEquityRatio", 0)
        )
        ...
        if income_data and len(income_data) > 0:
            ...
            metrics["operating_income"] = safe_float(
                latest_income.get("operatingIncome", 0)
            )
            metrics["interest_expense"] = safe_float(
                latest_income.get("interestExpense", 0)
            )

        if cash_flow_data and len(cash_flow_data) > 0:
            latest_cf = cash_flow_data[0]  # Most recent year
            metrics["operating_cash_flow"] = safe_float(
                latest_cf.get("operatingCashflow", 0)
            )
            metrics["free_cash_flow"] = safe_float(
                latest_cf.get("operatingCashflow", 0)
            ) - safe_float(latest_cf.get("capitalExpenditures", 0))
            metrics["capital_expenditures"] = safe_float(
                latest_cf.get("capitalExpenditures", 0)
            )
            metrics["dividends_paid"] = abs(
                safe_float(latest_cf.get("dividendPayout", 0))
            )
...
def show_dividend_screener_tab(
    df,
    monthly_data,
    currency,
    theme,
    ...
    **kwargs,
    ...
    with col2:
        time_period = st.selectbox(
            "Analysis Period", ["5 Years", "3 Years", "All Available"], index=0
        )
            ...
            with st.spinner(f"Fetching comprehensive data for {symbol}..."):
                ...
                if overview:
                    ...
                    metrics = calculate_financial_metrics(
                        overview, income_data, balance_data, cash_flow_data
                    )

                    # Process dividend history with proper yield calculation
                    div_history, annual_divs, quarterly_divs, yield_df = (
                        process_dividend_history(
                            dividend_data, daily_price_data, overview
                        )
                    )
                    ...
                    st.markdown("---")
                    st.subheader(
                        f"📋 Company Profile: {metrics['name']} ({metrics['symbol']})"
                    )
                    ...
                    with st.expander("📊 Detailed Company Information"):
                        ...
                        with col1:
                            ...
                            st.write(
                                f"**Shares Outstanding:** {metrics['shares_outstanding']:,.0f}"
                        ...
                        with col2:
                            st.write(
                                f"**52 Week High:** ${metrics['52_week_high']:.2f}"
                                if metrics["52_week_high"] > 0
                                else "**52 Week High:** N/A"
                            )
                            st.write(
                                f"**52 Week Low:** ${metrics['52_week_low']:.2f}"
                                if metrics["52_week_low"] > 0
                                else "**52 Week Low:** N/A"
                            )
                            st.write(
                                f"**Ex-Dividend Date:** {metrics['ex_dividend_date']}"
                            )
                            st.write(f"**Dividend Date:** {metrics['dividend_date']}")
                            st.write(
                                f"**Book Value:** ${metrics['book_value']:.2f}"
                        ...
                        if metrics["description"] != "N/A":
                            ...
                            st.write(
                                metrics["description"][:500] + "..."
                                if len(metrics["description"]) > 500
                                else metrics["description"]
                            )
                    ...
                    with analysis_tabs[0]:
                        ...
                        if income_data and len(income_data) > 0:
                                    ...
                                    income_chart_data.append(
                                        {
                                            ...
                                            "total_revenue": safe_float(
                                                report.get("totalRevenue", 0)
                                            )
                                            / 1e9,
                                            "gross_profit": safe_float(
                                                report.get("grossProfit", 0)
                                            )
                                            / 1e9,
                                            "operating_income": safe_float(
                                                report.get("operatingIncome", 0)
                                            )
                                            / 1e9,
                                            "net_income": safe_float(
                                                report.get("netIncome", 0)
                                            )
                                            / 1e9,
                                            "ebitda": safe_float(
                                                report.get("ebitda", 0)
                            ...
                            if income_chart_data:
                                income_df = pd.DataFrame(income_chart_data).sort_values(
                                    "year"
                                )
                                ...
                                fig_income.update_layout(
                                    ...
                                    xaxis_title="Year",
                                    yaxis=dict(
                                        title="Revenue ($ Billions)", side="left"
                                    ),
                                    yaxis2=dict(
                                        title="Profit ($ Billions)",
                                        side="right",
                                        overlaying="y",
                                    ),
                                ...
                                display_income_df = display_income_df.rename(
                                    columns={
                                        "year": "Year",
                                ...
                                st.dataframe(
                                    display_income_df, use_container_width=True
                                )
                        else:
                    ...
                    with analysis_tabs[1]:
                        ...
                        if balance_data and len(balance_data) > 0:
                                    ...
                                    balance_chart_data.append(
                                        {
                                            ...
                                            "total_assets": safe_float(
                                                report.get("totalAssets", 0)
                                            )
                                            / 1e9,
                                            "total_liabilities": safe_float(
                                                report.get("totalLiabilities", 0)
                                            )
                                            / 1e9,
                                            ...
                                            "current_assets": safe_float(
                                                report.get("totalCurrentAssets", 0)
                                            )
                                            / 1e9,
                                            ...
                                            / 1e9,
                                            "long_term_debt": safe_float(
                                                report.get("longTermDebt", 0)
                                            )
                                            / 1e9,
                                            "cash": safe_float(
                                                report.get(
                            ...
                            if balance_chart_data:
                                ...
                                # Calculate net debt
                                balance_df["net_debt"] = (
                                    balance_df["long_term_debt"] - balance_df["cash"]
                                )
                                fig_debt.add_trace(
                                    go.Scatter(
                                        x=balance_df["year"],
                                        ...
                                        name="Net Debt",
                                        line=dict(
                                            color="#f59e0b", width=3, dash="dash"
                                        ),
                                    )
                                )
                                ...
                                display_balance_df = display_balance_df.rename(
                                    columns={
                                        "year": "Year",
                                ...
                                st.dataframe(
                                    display_balance_df, use_container_width=True
                                )
                        else:
                    ...
                    with analysis_tabs[2]:
                        ...
                        if cash_flow_data and len(cash_flow_data) > 0:
                            ...
                            for report in cash_flow_data:
                                ...
                                if fiscal_year and int(fiscal_year) >= cutoff_year:
                                    operating_cf = safe_float(
                                        report.get("operatingCashflow", 0)
                                    )
                                    capex = safe_float(
                                        report.get("capitalExpenditures", 0)
                                    )
                                    free_cf = (
                                        operating_cf + capex
                                    ...
                                    cf_chart_data.append(
                                        {
                                            ...
                                            "investing_cash_flow": safe_float(
                                                report.get("cashflowFromInvestment", 0)
                                            )
                                            / 1e9,
                                            "financing_cash_flow": safe_float(
                                                report.get("cashflowFromFinancing", 0)
                                            )
                                            / 1e9,
                            ...
                            if cf_chart_data:
                                ...
                                st.plotly_chart(
                                    fig_cf_components, use_container_width=True
                                )
                                ...
                                if len(cf_df) > 0:
                                    ...
                                    with col1:
                                        coverage_ratio = (
                                            ...
                                            / latest_cf["dividends_paid"]
                                            if latest_cf["dividends_paid"] > 0
                                            else 0
                                        )
                                        st.metric(
                                    ...
                                    with col3:
                                        capex_intensity = (
                                            (
                                                latest_cf["capital_expenditures"]
                                                / metrics["revenue_ttm"]
                                                * 1e9
                                            )
                                            * 100
                                ...
                                display_cf_df = display_cf_df.rename(
                                    columns={
                                        "year": "Year",
                    ...
                    with analysis_tabs[3]:
                        ...
                        if div_history is not None and not div_history.empty:
                            # Filter data based on time period
                            filtered_history = div_history[
                                div_history["year"] >= cutoff_year
                            ]
                            filtered_annual = (
                                annual_divs[annual_divs["year"] >= cutoff_year]
                                if annual_divs is not None
                                else None
                            )
                            filtered_quarterly = (
                                quarterly_divs[quarterly_divs["year"] >= cutoff_year]
                                if quarterly_divs is not None
                                else None
                            )
                            ...
                            with col1:
                                if (
                                    filtered_annual is not None
                                    and not filtered_annual.empty
                                ):
                                    fig_annual = px.bar(
                                        filtered_annual,
                                        x="year",
                                    ...
                                    st.plotly_chart(
                                        fig_annual, use_container_width=True
                                    )

                            with col2:
                                if (
                                    filtered_quarterly is not None
                                    and not filtered_quarterly.empty
                                ):
                                    ...
                                    st.plotly_chart(
                                        fig_quarterly, use_container_width=True
                                    )
                            ...
                            if yield_df is not None and not yield_df.empty:
                                ...
                                filtered_yield = yield_df[
                                    yield_df["date"]
                                    >= pd.to_datetime(f"{cutoff_year}-01-01")
                                ]
                                ...
                                if not filtered_yield.empty:
                                    ...
                                    st.warning(
                                        "No dividend yield data available for the selected time period"
                                    )
                            else:
                                # Fallback to estimated yield if daily price data not available
                                if (
                                    filtered_history is not None
                                    and not filtered_history.empty
                                ):
                                    period_avg = filtered_history[
                                        "estimated_yield"
                                    ].mean()
                                    ...
                                    fig_yield = px.line(
                                        ...
                                        labels={
                                            "estimated_yield": "Estimated Dividend Yield (%)",
                                            "date": "Date",
                                        },
                                        template="plotly_white"
                                        if theme == "Light"
                            ...
                            if (
                                income_data
                                and len(income_data) > 0
                                and filtered_annual is not None
                            ):
                                payout_data = []
                                for report in income_data:
                                    fiscal_year = report.get("fiscalDateEnding", "")[:4]
                                ...
                                if payout_data:
                                    payout_df = pd.DataFrame(payout_data).sort_values(
                                        "year"
                                    )
                                    ...
                                    st.plotly_chart(
                                        fig_payout, use_container_width=True
                                    )
                            ...
                            if filtered_annual is not None and len(filtered_annual) > 1:
                                ...
                                for i in range(1, len(filtered_annual)):
                                    ...
                                    if previous_year_data["dividend"] > 0:
                                        growth = (
                                            (
                                                current_year_data["dividend"]
                                                - previous_year_data["dividend"]
                                            )
                                            / previous_year_data["dividend"]
                                        ) * 100
                                        growth_data.append(
                                ...
                                if growth_data:
                                    ...
                                    st.plotly_chart(
                                        fig_growth, use_container_width=True
                                    )
                            ...
                            with col1:
                                ...
                                st.metric(
                                    "Dividend Per Share",
                                    f"${metrics['dividend_per_share']:.2f}"
                            ...
                            with col2:
                                ...
                                st.metric(
                                    "Ex-Dividend Date", metrics["ex_dividend_date"]
                                )
                            ...
                            with col3:
                                ...
                                if "growth_data" in locals() and len(growth_data) > 0:
                                    avg_growth = sum(
                                        [g["growth"] for g in growth_data]
                                    ) / len(growth_data)
                                    st.metric(
                                        "Avg Dividend Growth", f"{avg_growth:.1f}%"
                                    )
                                else:
                            ...
                            with col4:
                                ...
                                if (
                                    "cf_chart_data" in locals()
                                    and len(cf_chart_data) > 0
                                ):
                                    latest_cf_data = cf_chart_data[-1]
                                    fcf_coverage = (
                                        latest_cf_data["free_cash_flow"]
                                        / latest_cf_data["dividends_paid"]
                                        if latest_cf_data["dividends_paid"] > 0
                                        else 0
                                    )
                                    st.metric(
                                        "FCF Coverage",
                                ...
                                years_paying = (
                                    len(filtered_annual)
                                    if filtered_annual is not None
                                    else 0
                                )
                    ...
                    with col2:
                        ...
                        if (
                            metrics["payout_ratio"] <= 60
                            and metrics["payout_ratio"] > 0
                        ):
