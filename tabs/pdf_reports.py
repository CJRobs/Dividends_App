...
def create_pdf_report(
    ...
    if period_type == "Monthly":
        ...
        if period_date.month == 12:
            end_date = period_date.replace(
                year=period_date.year + 1, month=1, day=1
            ) - timedelta(days=1)
        else:
            end_date = period_date.replace(
                month=period_date.month + 1, day=1
            ) - timedelta(days=1)
    ...
    if period_df.empty:
        story.append(
            Paragraph("No dividend data available for this period.", styles["Normal"])
        )
        doc.build(story)
        buffer.seek(0)
    ...
    top_stocks_table = Table(
        top_stocks_data, colWidths=[2.5 * inch, 1.5 * inch, 1 * inch]
    )
    top_stocks_table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#e8eaf6")),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#1a237e")),
    ...
    if period_type != "Monthly":
        ...
        period_breakdown = (
            period_df.groupby("Period")["Total"].agg(["sum", "count"]).round(2)
        )
        ...
        breakdown_table = Table(
            breakdown_data, colWidths=[2 * inch, 1.5 * inch, 1 * inch]
        )
...
def show_pdf_reports_tab(
    ...
    **kwargs,
    ...
    st.markdown(
        "Generate professional PDF reports for your dividend portfolio analysis."
    )
    ...
    if st.button("📋 Preview Report Content", type="secondary"):
        with st.spinner("Generating preview..."):
            ...
            if report_type == "Monthly":
                ...
                preview_title = (
                    f"Monthly Report Preview - {period_date.strftime('%B %Y')}"
                )
            elif report_type == "Quarterly":
                quarter = (period_date.month - 1) // 3 + 1
                start_month = (quarter - 1) * 3 + 1
                ...
                if quarter == 4:
                    ...
                    end_month = quarter * 3 + 1
                    end_date = period_date.replace(month=end_month, day=1) - timedelta(
                        days=1
                    )
                preview_title = (
                    f"Quarterly Report Preview - Q{quarter} {period_date.year}"
                )
            else:  # Yearly
                start_date = period_date.replace(month=1, day=1)
                end_date = period_date.replace(month=12, day=31)
            ...
            with col4:
                avg_dividend = period_df["Total"].mean()
                st.metric(
                    "Avg per Transaction", format_currency(avg_dividend, currency)
                )

            # Top performers
            st.subheader("Top Performing Stocks")
            top_stocks = (
                period_df.groupby("Name")["Total"].agg(["sum", "count"]).round(2)
            )
            top_stocks.columns = ["Total Dividends", "Transactions"]
            top_stocks = top_stocks.sort_values(
                "Total Dividends", ascending=False
            ).head(10)
            top_stocks["Total Dividends"] = top_stocks["Total Dividends"].apply(
    ...
    if st.button("📄 Generate PDF Report", type="primary"):
        try:
            ...
            st.info(
                "Make sure you have the required dependencies installed: `pip install reportlab`"
            )
