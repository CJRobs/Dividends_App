...
def show_monthly_analysis_tab(
    ...
    **kwargs,
    ...
    monthly_by_year = df.pivot_table(
        index="MonthName", columns="Year", values="Total", aggfunc="sum"
    ).reset_index()
    ...
    years = [
        col for col in monthly_by_year.columns if col not in ["MonthName", "MonthNum"]
    ]

    fig_monthly_line = go.Figure()

    for year in years:
        fig_monthly_line.add_trace(
            go.Scatter(
    ...
    monthly_pivot = df.pivot_table(
        index="Year", columns="MonthName", values="Total", aggfunc="sum"
    ).fillna(0)
    ...
    fig_heatmap.update_layout(
        height=500, xaxis=dict(title="Month"), yaxis=dict(title="Year")
    )
    ...
    if selected_month == "All Months":
        # Show all months
        monthly_by_company = (
            filtered_df.groupby(["MonthName", "Month", "Name"])["Total"]
            .sum()
            .reset_index()
        )
        monthly_by_company["MonthNum"] = monthly_by_company["MonthName"].apply(
            lambda x: month_order.index(x)
        )
        ...
        fig_monthly_company.update_layout(
            height=500,
            xaxis=dict(categoryorder="array", categoryarray=month_order, title="Month"),
            yaxis=dict(title=f"Dividend Amount ({currency})"),
            legend=dict(title="Company", orientation="v"),
            barmode="stack",
        )
    else:
        # Show specific month across all years
        monthly_by_company = (
            filtered_df.groupby(["Year", "Name"])["Total"].sum().reset_index()
        )
    ...
    fig_monthly_company.update_layout(font=dict(color="black"))
    ...
    coverage_percent = (
        (most_recent_month_amount / monthly_expenses) * 100
    ...
    st.metric("Current Coverage", f"{coverage_percent:.1f}%", delta=None)
